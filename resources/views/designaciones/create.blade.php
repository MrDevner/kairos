...
<div class="card" style="max-width:760px">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('designaciones.store') }}" id="form-designacion">
            @csrf
            ...
            <div class="row g-3">
                    ...
                    <select id="sel-usuario-desig" name="id_usuario"
                        ...
                        @if($oldUser)
                            <option value="{{ $oldUser->id }}" selected>
                                {{ $oldUser->apellidos }}, {{ $oldUser->nombres }}{{ $oldUser->documento ? ' ('.$oldUser->documento.')' : '' }}
                            </option>
                        @endif
                    </select>
                    ...
                    <select name="id_cargo" id="id_cargo" class="form-select form-select-sm @error('id_cargo') is-invalid @enderror" required>
                        ...
                        @foreach($cargos as $cargo)
                            <option value="{{ $cargo->id }}"
                                    ...
                                    @selected(old('id_cargo') == $cargo->id)>
                                {{ $cargo->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    {{-- Note: filter this select via JS when id_institucion changes --}}
                    <select name="id_dependencia" id="id_dependencia" class="form-select form-select-sm @error('id_dependencia') is-invalid @enderror">
                        ...
                        @foreach($dependencias as $dep)
                            <option value="{{ $dep->id }}"
                                    ...
                                    @selected(old('id_dependencia') == $dep->id)>
                                {{ $dep->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="resolucion" class="form-control form-control-sm @error('resolucion') is-invalid @enderror"
                           value="{{ old('resolucion') }}" maxlength="100" placeholder="Nº de resolución">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="horas_semanales_efectivas" class="form-control form-control-sm @error('horas_semanales_efectivas') is-invalid @enderror"
                           value="{{ old('horas_semanales_efectivas') }}" min="1" max="168" step="0.5">
...
<script>
    ...
    // Mostrar horas del cargo seleccionado
    ...
    // Filtrar dependencias por institución seleccionada
    document.getElementById('id_institucion').addEventListener('change', function () {
        ...
        if (instId) {
            select.options[0].textContent = '— Seleccione una dependencia —';
        } else {
            select.options[0].textContent = '— Seleccione primero una institución —';
        }
    });

    // Aplicar filtro inicial si hay valor de old()
