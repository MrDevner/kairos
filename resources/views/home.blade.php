...
{{-- ══ DASHBOARD INSTITUCIONAL (cuando hay institución seleccionada) ══════ --}}
@isset($instActiva)
...
@elseif($user->permisos()->administrador()->tieneTodosLosPermisos())
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <a href="{{ route('instituciones.index') }}" class="text-decoration-none">
    ...
    <div class="row g-3">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-activity me-2"></i>Actividad del sistema — últimos 30 días
                </div>
                <div class="card-body">
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header text-bg-danger border-0">
                    <i class="bi bi-exclamation-triangle-fill me-2"></i>Errores críticos
                </div>
...
@elseif($user->tieneRolEnInstitucion('Jefe de Personal', $instId) || $user->tieneRolEnInstitucion('Departamento Personal', $instId))
    ...
    <div class="row g-3">
        <div class="col-md-7">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-bar-chart-fill me-2"></i>Asistencia semanal
                </div>
        ...
        <div class="col-md-5">
            <div class="card h-100">
                <div class="card-header text-bg-danger border-0">
                    <i class="bi bi-exclamation-triangle-fill me-2"></i>Atención urgente hoy
                </div>
                <div class="card-body p-0" style="max-height:260px;overflow-y:auto">
                    ...
                    @empty
                        <div class="px-3 py-3 text-muted small text-center">
                            <i class="bi bi-check-circle text-success"></i> Sin novedades urgentes
                        </div>
                    @endforelse
                </div>
            </div>
...
{{-- ══ DIRECTOR ADMINISTRATIVO / AUDITOR (sin institución) ════════════════ --}}
@elseif($user->tieneRolEnInstitucion('Director Administrativo', $instId) || $user->tieneRolEnInstitucion('Auditor', $instId))
    <h5 class="fw-bold mb-4">
        <i class="bi bi-speedometer2 me-2" style="color:var(--azul)"></i>Dashboard
    </h5>
    <div class="row g-3 mb-4">
        <div class="col-md-4">
            <div class="card text-center">
                <div class="card-body py-4">
                    <i class="bi bi-people fs-1" style="color:var(--azul)"></i>
                    <div class="fs-2 fw-bold mt-1">{{ $stats['personal'] ?? 0 }}</div>
                    <div class="text-muted">Personal total</div>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card text-center">
                <div class="card-body py-4">
                    ...
                    <div class="fs-2 fw-bold mt-1 text-success">{{ $stats['presentes'] ?? 0 }}</div>
        ...
        <div class="col-md-4">
            <div class="card text-center">
                <div class="card-body py-4">
                    ...
                    <div class="fs-2 fw-bold mt-1 text-danger">{{ $stats['ausentes'] ?? 0 }}</div>
                    <div class="text-muted">Ausentes hoy</div>
                </div>
            </div>
        </div>
    </div>
...
@else
    <h5 class="fw-bold mb-4">
        <i class="bi bi-speedometer2 me-2" style="color:var(--azul)"></i>
        Mi resumen de hoy — {{ now()->isoFormat('dddd D [de] MMMM') }}
    </h5>
    ...
    <div class="row g-3">
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-file-text me-2"></i>Mi DDJJ vigente
                </div>
                <div class="card-body">
                    @if($ddjjVigente ?? null)
                        <p class="mb-1 small text-muted">
                            Vigente desde {{ $ddjjVigente->fecha_inicio->format('d/m/Y') }}
                        </p>
                        <table class="table table-sm table-borderless mb-0">
                            @foreach($ddjjVigente->horarios as $h)
                                <tr>
                                    <td class="text-capitalize fw-semibold" style="width:110px">
                                        {{ $h->dia_semana }}
                                    </td>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-megaphone me-2"></i>Últimos avisos / licencias
                </div>
                <div class="card-body p-0">
...
@elseif(!isset($instActiva) && ($user->tieneRolEnInstitucion('Jefe de Personal', $instId) || $user->tieneRolEnInstitucion('Departamento Personal', $instId)))
